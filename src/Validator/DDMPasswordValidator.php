...
class DDMPasswordValidator extends DDMValidator
{
    ...
    public function validate(mixed $value): bool
    {
        $password = '';
        if (is_array($value)) {
            $password = array_key_exists(0, $value) && is_string($value[0]) ? $value[0] : '';
        ...
        if (is_array($value) && count($value) >= 2) {
            $first = array_key_exists(0, $value) && is_string($value[0]) ? $value[0] : '';
            $second = array_key_exists(1, $value) && is_string($value[1]) ? $value[1] : '';

            if (('' === trim($first)) xor ('' === trim($second))) {
